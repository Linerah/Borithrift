...
class TestProfile(unittest.TestCase):
    ...
    def test_init(self):
        ...
        test_items = [self.victor_profile.user_items, self.kevin_profile.user_items, self.josue_profile.user_items]
    ...
    def test_init_types(self):
        ...
        for items in test_items: 
             with self.subTest():
                self.assertRaises(TypeError, Profile, 'someone', items, 4.25, 27, "../flag.png")
    ...
    def test_add_item_to_sell(self):
        ...
        for item in test_items: 
             with self.subTest():
                self.assertRaises(TypeError, self.victor_profile.Add_Item_to_Sell(item))
        ...
        self.assertRaises(ValueError, self.kevin_profile.Add_Item_to_Sell(victor_item1))
    ...
    def test_remove_item(self):
        ...
        for item in test_items: 
             with self.subTest():
                self.assertRaises(TypeError, self.victor_profile.Remove_Item(item))

        # TODO: the test below fails
        # should not remove an item if it is owned by another user
        victor_item1 = Item("Yosemite Tee",15.00,"L","Sportswear","Men","Basic T-shirt for everyday use","../images/yosemite_tee.png", self.victor)
        self.assertRaises(ValueError, self.kevin_profile.Remove_Item(victor_item1))
    ...
    def test_review_score(self):
        ...
        for new_review in test_new_reviews:
             with self.subTest():
                self.assertRaises(TypeError, self.josue_profile.Review_Score(new_review))

        # reviews - must be between 0 and 5
        test_new_reviews = [ -1, 6] 
        for new_review in test_new_reviews:
             with self.subTest():
                self.assertRaises(ValueError, self.josue_profile.Review_Score(new_review))
        ...
        for profile, new_review, result in test_new_reviews:
            with self.subTest():
                self.assertAlmostEqual(profile.Review_Score(new_review), result)